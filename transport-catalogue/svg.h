...
namespace svg {
  ...
  inline std::ostream & operator << (std::ostream & out, const Rgba & rgba) {
    using namespace std::literals;
    out << "rgba("
    s << static_cast < int > (rgba.red) << ',' << static_cast < int > (rgba.green) << ',' << static_cast < int > (rgba.blue) << ',' << rgba.opacity << ')';
    return out;
  }
  ...
  inline std::ostream& operator << (std::ostream & out, const Color & color) {
    std::visit([ & out](const auto & color) {out << color;}, color);
    return out;
  }
  ...
  class Object {
    public: void Render(const RenderContext & context) const;
  ...
  inline std::ostream & operator << (std::ostream & out, StrokeLineCap value) {
    using namespace std::literals;
    if (value == StrokeLineCap::BUTT) {
      out << "butt"s;
    }
    if (value == StrokeLineCap::ROUND) {
      out << "round"s;
    }
    if (value == StrokeLineCap::SQUARE) {
      out << "square"s;
    }
    return out;
  }
  ...
  template < typename Owner >
    class PathProps {
      ...
      void RenderAttrs(std::ostream & out) const {
        ...
        using namespace std::literals;
        RenderOptionalAttr(out, " fill"
          sv, fill_color_);
        RenderOptionalAttr(out, " stroke"
          sv, stroke_color_);
        RenderOptionalAttr(out, " stroke-width"
          sv, stroke_width_);
        RenderOptionalAttr(out, " stroke-linecap"
          sv, stroke_line_cap_);
        RenderOptionalAttr(out, " stroke-linejoin"
          sv, stroke_line_join_);
      }
  ...
  class Circle: public Object, public PathProps < Circle > {
    ...
    private: void RenderObject(const RenderContext & context) const override;
  ...
  class Text: public Object, public PathProps < Text > {
    public: Text & SetPosition(Point pos);
  ...
  class Drawable {
    public: virtual void Draw(ObjectContainer & container) const = 0;
    virtual~Drawable() = default;
  };

  class Document: public ObjectContainer {
    public: void AddPtr(std::unique_ptr < Object > && obj) override;
    void Render(std::ostream & out) const;

    private: std::vector < std::unique_ptr < Object >> objects_;
  };
